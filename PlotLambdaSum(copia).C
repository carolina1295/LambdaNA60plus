...
void PlotLambdaSumcopia(){
	...
	TFile *foutLambda=new TFile("histogramcheck.root","RECREATE");
...
/*	//Distribuzione 3D per efficienza Lambda Bar
	...
	*/
	...
	TH2F *hResPxVsPtotpionRel = new TH2F("hResPxVsYpionRel", "Res Px vs Y Relativa", 200, -0.08, 0.08, 200, 0, 50);
	TH2F *hResPyVsPtotpionRel = new TH2F("hResPyVsYpionRel", "Res Py vs Y Relativa", 200, -0.08, 0.08, 200, 0, 50);
	TH2F *hResPzVsPtotpionRel = new TH2F("hResPzVsYpionRel", "Res Pz vs Y Relativa", 200, -0.08, 0.08, 200, 0, 50);
	TH2F *hResPxVsPtotprotRel = new TH2F("hResPxVsYprotRel", "Res Px vs Y Relativa", 200, -0.08, 0.08, 200, 0, 50);
	TH2F *hResPyVsPtotprotRel = new TH2F("hResPyVsYprotRel", "Res Py vs Y Relativa", 200, -0.08, 0.08, 200, 0, 50);
	TH2F *hResPzVsPtotprotRel = new TH2F("hResPzVsYprotRel", "Res Pz vs Y Relativa", 200, -0.08, 0.08, 200, 0, 50);
	...
	for(int i=0 ; i<events; i++) {
		...
		double Ptot=sqrt(pGenLambda[0]*pGenLambda[0]+pGenLambda[1]*pGenLambda[1]+pGenLambda[2]*pGenLambda[2]);
		...
		if(nfaketrkprot == 0 && nfaketrkpion == 0) {
			...
			hResPxVsPtotpionRel->Fill((pRecPion[0]-pGenPion[0])/pGenPion[0],Ptot);
			hResPyVsPtotpionRel->Fill((pRecPion[1]-pGenPion[1])/pGenPion[1],Ptot);
			hResPzVsPtotpionRel->Fill((pRecPion[2]-pGenPion[2])/pGenPion[2],Ptot);

			hResPxVsPtotprotRel->Fill((pRecProt[0]-pGenProt[0])/pGenProt[0],Ptot);
			hResPyVsPtotprotRel->Fill((pRecProt[1]-pGenProt[1])/pGenProt[1],Ptot);
			hResPzVsPtotprotRel->Fill((pRecProt[2]-pGenProt[2])/pGenProt[2],Ptot);
	...
	for(int i=0 ; i<eventsBar; i++) {
		...
		if(nfaketrkprotBar == 0 && nfaketrkpionBar == 0) {
			...
			double PtotBar=sqrt(pGenLambdaBar[0]*pGenLambdaBar[0]+pGenLambdaBar[1]*pGenLambdaBar[1]+pGenLambdaBar[2]*pGenLambdaBar[2]);

			//Resolution on P

			hResPxVsPtotpionRel->Fill((pRecPionBar[0]-pGenPionBar[0])/pGenPionBar[0],PtotBar);
			hResPyVsPtotpionRel->Fill((pRecPionBar[1]-pGenPionBar[1])/pGenPionBar[1],PtotBar);
			hResPzVsPtotpionRel->Fill((pRecPionBar[2]-pGenPionBar[2])/pGenPionBar[2],PtotBar);

			hResPxVsPtotprotRel->Fill((pRecProtBar[0]-pGenProtBar[0])/pGenProtBar[0],PtotBar);
			hResPyVsPtotprotRel->Fill((pRecProtBar[1]-pGenProtBar[1])/pGenProtBar[1],PtotBar);
			hResPzVsPtotprotRel->Fill((pRecProtBar[2]-pGenProtBar[2])/pGenProtBar[2],PtotBar);
	...
	hResPxVsPtotpionRel->Write();
	hResPyVsPtotpionRel->Write();
	hResPzVsPtotpionRel->Write();

	hResPxVsPtotprotRel->Write();
	hResPyVsPtotprotRel->Write();
	hResPzVsPtotprotRel->Write();
	...
	TCanvas* c5piRelPtot=new TCanvas("c5piRelPtot");
	ProjectionforBin(hResPxVsPtotpionRel, "#Lambda+#bar{#Lambda} pion ResPx vs Ptot Rel", "Ptot", c5piRelPtot, foutLambda);
	TCanvas* c6piRelPtot=new TCanvas("c6piRelPtot");
	ProjectionforBin(hResPyVsPtotpionRel, "#Lambda+#bar{#Lambda} pion ResPy vs Ptot Rel", "Ptot", c6piRelPtot, foutLambda);
	TCanvas* c7piRelPtot=new TCanvas("c7piRelPtot");
	ProjectionforBin(hResPzVsPtotpionRel, "#Lambda+#bar{#Lambda} pion ResPz vs Ptot Rel", "Ptot", c7piRelPtot, foutLambda);

	TCanvas* c5pRelPtot=new TCanvas("c5pRelPtot");
	ProjectionforBin(hResPxVsPtotprotRel, "#Lambda+#bar{#Lambda} prot ResPx vs Ptot Rel", "Ptot", c5pRelPtot, foutLambda);
	TCanvas* c6pRelPtot=new TCanvas("c6pRelPtot");
	ProjectionforBin(hResPyVsPtotprotRel, "#Lambda+#bar{#Lambda} prot ResPy vs Ptot Rel", "Ptot", c6pRelPtot, foutLambda);
	TCanvas* c7pRelPtot=new TCanvas("c7pRelPtot");
	ProjectionforBin(hResPzVsPtotprotRel, "#Lambda+#bar{#Lambda} prot ResPz vs Ptot Rel", "Ptot", c7pRelPtot, foutLambda);
