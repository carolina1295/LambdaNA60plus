...
void CutPlot(bool ScanX2){
    ...
    TFile filinBkg("fntBkg5000.root");
    ...
    int bmin = axis->FindBin(meanLow); //in your case xmin=-1.5
...
lat2->DrawLatex(0.5,0.55,Form("Significance(3#sigma) %f #pm %f",(sqrt(5*1e9)*Scheck*1e-9/5)/(sqrt((Scheck*1e-9/5)+(Bcheck*1e-9/5))),sqrt((sqrt(5*1e9)*Scheck*1e-9/5)/(sqrt((Scheck*1e-9/5)+(Bcheck*1e-9/5))))));
fout->cd();
hSum->Write();
