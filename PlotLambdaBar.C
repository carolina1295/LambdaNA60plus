...
TFile *hLambda=new TFile("LambdaBar-Signal-histos.root");
...
//Lettura Ntupla 
 	TFile filin("ntuplaLambdaBar.root");
  ...
  TNtuple *variables = (TNtuple*)filin.Get("ntLambdaBar");
	...
	TFile *foutLambda=new TFile("histogramLambdaBar.root","RECREATE");
...
//	TH1F hmassnfake("hMassinrecnfakeprot", "Massinv for N fake prot", 3000, 0,3);
	...
	ProjectionforBinMassInv(hMassinreczP, "#Lambda bar Inv Mass vs Lz", "zP", c01, foutLambda);
	TCanvas* c1=new TCanvas("c1");
	ProjectionforBinMassInv(hMassinrecY, "#Lambda bar Inv Mass vs Y", "Y", c1, foutLambda);
	...
	ProjectionforBin(hResVxVsY, "#Lambda bar ResX vs Y", "Y", c2, foutLambda);
	TCanvas* c3=new TCanvas("c3");
	ProjectionforBin(hResVyVsY, "#Lambda bar ResY vs Y", "Y", c3, foutLambda);
	TCanvas* c4=new TCanvas("c4");
	ProjectionforBin(hResVzVsY, "#Lambda bar ResZ vs Y", "Y", c4, foutLambda);
	/*TCanvas* c5=new TCanvas("c5");
	ProjectionforBin(hResVxVsLz, "#Lambda bar ResX vs Lz", "Lz", c5, foutLambda);
	TCanvas* c6=new TCanvas("c6");
	ProjectionforBin(hResVyVsLz, "#Lambda bar ResY vs Lz", "Lz", c6, foutLambda);
	TCanvas* c7=new TCanvas("c7");
	ProjectionforBin(hResVzVsLz, "#Lambda bar ResZ vs Lz", "Lz", c7, foutLambda);*/
...
void ProjectionforBin(TH2F* hist2D, char* graphTitle , char* Xaxisname, TCanvas *c0, TFile *f){
  ...
  for(Int_t jj=1; jj<hist2D->GetNbinsY(); jj++){
    TH1D *hRes = (TH1D *)hist2D->ProjectionX(Form("%sBin%d",hist2D->GetName(),jj), jj, jj);
		//hRes->SetDirectory(0);
    //hRes->Draw();
    if(hRes->GetSumOfWeights()>45){
			count++;
			c0->cd(count);
			c0->SetTitle(Form("#Lambda Bar %s",hist2D->GetName()));
			//new TCanvas();
			hRes->SetDirectory(0);
    	hRes->Draw();
      Double_t rmsh=hRes->GetRMS();
      Double_t meanh=hRes->GetMean();
			Double_t Errrmsh=hRes->GetRMSError();
      Double_t Errmeanh=hRes->GetMeanError();

			hRes->Fit("gaus","","",meanh-5*rmsh,meanh+5*rmsh);
    	TF1* fg=(TF1*)hRes->GetListOfFunctions()->FindObject("gaus");
...
void ProjectionforBinMassInv(TH2F* hist2D, char* graphTitle , char* Xaxisname, TCanvas *c0, TFile *f){
  ...
  for(Int_t jj=1; jj<hist2D->GetNbinsY(); jj++){
    ...
    TH1D *hInvMass = (TH1D *)hist2D->ProjectionX(Form("hMassBin%d",jj), jj, jj);
    ...
    if(hInvMass->GetSumOfWeights()>45){
      ...
      Double_t Errmeanh=hInvMass->GetMeanError();
		...
		TF1* fg=((TF1*)(gROOT->GetFunction("gaus")));
			...
			fg->SetParameter(2,0.003);
			fg->SetParameter(0,hInvMass->GetMaximum());
      hInvMass->Fit("gaus","B","",meanh-5*rmsh,meanh+5*rmsh);

      Double_t meang=fg->GetParameter(1);
      Double_t emeang=fg->GetParError(1);
      Double_t sigmag=fg->GetParameter(2);
      Double_t esigma=fg->GetParError(2);
      printf("zP interval %f-%f   mean=%f+-%f  sigma=%f+-%f\n",
	    	hist2D->GetYaxis()->GetBinLowEdge(jj),
	    	hist2D->GetYaxis()->GetBinUpEdge(jj),
