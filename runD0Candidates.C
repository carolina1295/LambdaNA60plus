...
void GenerateD0SignalCandidates(Int_t nevents = 100000, 
 ...
 TFile *fntuplaD0 = new TFile("ntuplaD0.root", "recreate");
  ...
  Float_t arrnt[11];
	float nfaketrk = 0, nfaketrkKaon = 0, nfaketrkpion = 0;
  for (Int_t iev = 0; iev < nevents; iev++){
    ...
    Double_t secvertgenPi[3]={0.,0.,0.};
		Double_t RespxyzKaon[3] = {0.,0.,0.}, Respxyzpion[3] = {0.,0.,0.};
	...
	// daughters that can be reconstructed: K and pi
				...
				if (kf == 321){
					// Kaon daughter
					nfaketrkKaon = trw->GetNFakeITSHits();
					ptK = iparticle1->Pt();
					yK = iparticle1->Y();
					...
					RespxyzKaon[0]=(pxyz[0]-iparticle1->Px());
					RespxyzKaon[1]=(pxyz[1]-iparticle1->Py());
					RespxyzKaon[2]=(pxyz[2]-iparticle1->Pz());
    ...
    Double_t  massRecD=parent.M(); //massainvariante ricostruita
    ...
    hResVzVsY->Fill(residVz, yRecD);
		...
		variable[18]=(float)(daurec[1].Py());
		variable[19]=(float)(daurec[2].Pz());
		variable[20]=(float)(daurec[1].Px());
		variable[21]=(float)(daurec[1].Py());
		variable[22]=(float)(daurec[1].Pz());
