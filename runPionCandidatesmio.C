...
//istogrammi dei prodotti che saranno salvati in decayhistos.root
  ...
  TFile *fout = new TFile("PION-Signal-histos.root", "recreate");
...
//Magnetic field and detector parameters
 TVirtualMagField* fld = TGeoGlobalMagField::Instance()->GetField();
  ...
  Int_t pdgParticle = 211;// define PION particle
  
  TH1D* hLzRec0fake=new TH1D("hLzRec0fake", "Lz Rec 0 fake", 200, 5, 50);
  TH1D* hLzRec=new TH1D("hLzRec", "Lz Rec", 200, 5, 50);
...
//histograms for checking reconstruction (quindi tieni le righe di codice nel passaggio rivelatore)
	TH1D* hPrecXpion = new TH1D("hPrecXpion", "P_{Xrec} pion", 300, -5, 5);//width=0.01
	TH1D* hPrecYpion = new TH1D("hPrecYpion", "P_{Yrec} pion", 300, -2, 2);//width=0.01
	TH1D* hPrecZpion = new TH1D("hPrecZpion", "P_{Zrec} pion", 300, 0, 50);//width=0.01
	TH1D* hPrecgenXpion = new TH1D("hPrecgenXpion", "P_{Xrec}-P_{Xgen} pion", 300, -0.5, 0.5);//width=0.01
	TH1D* hPrecgenYpion = new TH1D("hPrecgenYpion", "P_{Yrec}-P_{Ygen} pion", 300, -0.5, 0.5);//width=0.01
	TH1D* hPrecgenZpion = new TH1D("hPrecgenZpion", "P_{Zrec}-P_{Zgen} pion", 300, -0.5, 0.5);//width=0.01
	TH2F* hResPxVsYpion = new TH2F("hResPxVsYpion", "P_{Xrec}-P_{Xgen}  vs Y pion", 300, -0.5 , 0.5,60, 0., 6);//width=0.01
	TH2F* hResPyVsYpion = new TH2F("hResPyVsYpion", "P_{Yrec}-P_{Ygen}  vs Y pion", 300, -0.5 , 0.5,60, 0., 6);
	TH2F* hResPzVsYpion = new TH2F("hResPzVsYpion", "P_{Zrec}-P_{Zgen}  vs Y pion", 300, -0.5 , 0.5,60, 0., 6);
	TH2F* hResPxVsYpionRel = new TH2F("hResPxVsYpionRel", "P_{Xrec}-P_{Xgen}  vs Y pion relativa", 300, -0.5 , 0.5,60, 0., 6);//width=0.01
	TH2F* hResPyVsYpionRel = new TH2F("hResPyVsYpionRel", "P_{Yrec}-P_{Ygen}  vs Y pion relativa", 300, -0.5 , 0.5,60, 0., 6);
	TH2F* hResPzVsYpionRel = new TH2F("hResPzVsYpionRel", "P_{Zrec}-P_{Zgen}  vs Y pion relativa", 300, -0.5 , 0.5,60, 0., 6);
//Nfake
...
//efficiency
      ...
      if (det->SolveSingleTrack(pDecDau->Pt(), pDecDau->Rapidity(), pDecDau->Phi(), mass, crg, vprim[0], vprim[1], vprim[2], 0, 1, 99)){
        ...
        if (trw){
          if (trw->GetNormChi2(kTRUE) < ChiTot){
            ...
            recProbe[0] = *trw; 
            hPrecXpion->Fill(pxyz[0]);
            hPrecYpion->Fill(pxyz[1]);
            hPrecZpion->Fill(pxyz[2]);	
            hPrecgenXpion->Fill(pxyz[0]-pxGenD);
            hPrecgenYpion->Fill(pxyz[1]-pyGenD);
            hPrecgenZpion->Fill(pxyz[2]-pzGenD);
            Respxyzpion[0]=(pxyz[0]-pxGenD);
            Respxyzpion[1]=(pxyz[1]-pyGenD);
            Respxyzpion[2]=(pxyz[2]-pzGenD);
            if(nfaketrk == 0){
              hResPxVsYpion->Fill(Respxyzpion[0],yGenD);
              hResPyVsYpion->Fill(Respxyzpion[1],yGenD);
              hResPzVsYpion->Fill(Respxyzpion[2],yGenD);
              hResPxVsYpionRel->Fill(Respxyzpion[0]/pxyz[0],yGenD);
              hResPyVsYpionRel->Fill(Respxyzpion[1]/pxyz[1],yGenD);
              hResPzVsYpionRel->Fill(Respxyzpion[2]/pxyz[2],yGenD);
            }
            countrec++;
          }		
        }
  ...
  // hyPi->Draw();
  */
  TFile fout2("DecayHistosPIONpos.root", "RECREATE");
